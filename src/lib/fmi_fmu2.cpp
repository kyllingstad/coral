...
namespace
{
    ...
    // variable IDs.
    std::vector<fmi2_value_reference_t> FMIValueReferences(
        gsl::span<const coral::model::VariableID> id,
        const FMU2& fmu)
    {
        std::vector<fmi2_value_reference_t> vr(id.size());
...
void SlaveInstance2::GetRealVariables(
    ...
    gsl::span<double> values) const
{
    ...
    static_assert (std::is_same<double, fmi2_real_t>::value, "type size mismatch");
...
void SlaveInstance2::GetIntegerVariables(
    gsl::span<const coral::model::VariableID> variables,
    gsl::span<int> values) const
{
    CORAL_INPUT_CHECK(variables.size() == values.size());
    const auto valRefs = FMIValueReferences(variables, *m_fmu);
    static_assert (std::is_same<int, fmi2_integer_t>::value, "type size mismatch");
...
void SlaveInstance2::GetBooleanVariables(
    gsl::span<const coral::model::VariableID> variables,
...
void SlaveInstance2::GetStringVariables(
    gsl::span<const coral::model::VariableID> variables,
...
bool SlaveInstance2::SetRealVariables(
    gsl::span<const coral::model::VariableID> variables,
...
bool SlaveInstance2::SetIntegerVariables(
    gsl::span<const coral::model::VariableID> variables,
...
bool SlaveInstance2::SetBooleanVariables(
    gsl::span<const coral::model::VariableID> variables,
    gsl::span<const bool> values)
{
    ...
    std::vector<fmi2_boolean_t> fmiValues(values.size());
    for (int i = 0; i < values.size(); ++i) {
        fmiValues[i] = values[i] ? fmi2_true : fmi2_false;
    }
...
bool SlaveInstance2::SetStringVariables(
    gsl::span<const coral::model::VariableID> variables,
